...
you can import this file and its functions directly:
    ...
    - jupyter notebook: include the lines `%autoreload 2` and `import checkpoint1b.py as c1b`
...
# checkpoint1b.py not preprocess
# add pandas 
# check if seaborn is there. yes.
...
def fix_caffeine(df):
    ...
    mode_df=df['Caffeine (mg)'].mode().iloc[0]
    for i in [np.nan,'varies','Varies']:
        df['Caffeine (mg)']=df['Caffeine (mg)'].replace(i,mode_df)
...
def standardize_names(df):
    ...
    for i in range(len(col)):
        col[i]=col[i].replace('(','').replace(')','').lower()
    df.columns=col
...
def fix_strings(df, col):
    ...
    df[col]=df[col].apply(lambda x: re.sub(r'\W+', ' ', x).lower())
